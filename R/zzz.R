.onLoad <- function(libname, pkgname){
  blockr::register_block(
    new_cardinal02_block,
    "Cardinal 02",
    "A Cardinal 02 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal02_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal03_block,
    "Cardinal 03",
    "A Cardinal 03 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal03_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal05_block,
    "Cardinal 05",
    "A Cardinal 05 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal05_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal06_block,
    "Cardinal 06",
    "A Cardinal 06 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal06_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal07_block,
    "Cardinal 07",
    "A Cardinal 07 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal07_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal08_block,
    "Cardinal 08",
    "A Cardinal 08 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal08_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal09_block,
    "Cardinal 09",
    "A Cardinal 09 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal09_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal10_block,
    "Cardinal 10",
    "A Cardinal 10 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal10_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal11_block,
    "Cardinal 11",
    "A Cardinal 11 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal11_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal12_block,
    "Cardinal 12",
    "A Cardinal 12 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal12_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal13_block,
    "Cardinal 13",
    "A Cardinal 13 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal13_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal14_block,
    "Cardinal 14",
    "A Cardinal 14 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal14_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal15_block,
    "Cardinal 15",
    "A Cardinal 15 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal15_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal16_block,
    "Cardinal 16",
    "A Cardinal 16 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal16_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal17_block,
    "Cardinal 17",
    "A Cardinal 17 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal17_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal18_block,
    "Cardinal 18",
    "A Cardinal 18 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal18_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal20_block,
    "Cardinal 20",
    "A Cardinal 20 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal20_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal21_block,
    "Cardinal 21",
    "A Cardinal 21 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal21_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal22_block,
    "Cardinal 22",
    "A Cardinal 22 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal22_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal32_block,
    "Cardinal 32",
    "A Cardinal 32 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal32_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal33_block,
    "Cardinal 33",
    "A Cardinal 33 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal33_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal34_block,
    "Cardinal 34",
    "A Cardinal 34 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal34_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal35_block,
    "Cardinal 35",
    "A Cardinal 35 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal35_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal36_block,
    "Cardinal 36",
    "A Cardinal 36 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal36_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_cardinal38_block,
    "Cardinal 38",
    "A Cardinal 38 table",
    input = "data.frame",
    output = "list",
    package = pkgname,
    classes = c("cardinal38_block", "rtables_block", "submit_block")
  )

  blockr::register_block(
    new_relabel_block,
    "Relabel block",
    "Change columns label attributes",
    input = "data.frame",
    output = "data.frame",
    package = pkgname,
    classes = c("relabel_block", "transform_block", "submit_block")
  )

  blockr::register_block(
    new_random_cdisc_data_block,
    "Random CDISC data",
    "Use random CDISC datasets",
    input = NA_character_,
    output = "data.frame",
    package = pkgname,
    classes = c("random_cdisc_block", "data_block")
  )
}
